...
cp -R "$HOME_DIR"/.cache/bower "$HOME_DIR"/bower_cache
...
for FILE_PATH in "$HOME_DIR"/.cache/bower/packages/*/*; do
  ...
  PACKAGE_FULL_PATH=${FILE_PATH##$SOURCE}
  DEST="$HOME_DIR"/bower/packages/

  PACKAGE=${PACKAGE_FULL_PATH%%/*}
  VERSION=${PACKAGE_FULL_PATH##*/}

  if [ ! -x "$DEST"/"$PACKAGE"/"$VERSION" ]; then
    mkdir -p "$DEST"/"$PACKAGE"
    cp -R "$FILE_PATH" "$DEST"/"$PACKAGE"
  fi
done
